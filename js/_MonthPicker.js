    ...
    $.widget("KidSysco.MonthPicker", {
        ...
        Open: function(event) {
            ...
            if (!_opts.Disabled && !this._visible) {
                ...
                if (this.options.isYearPicker) {
                    this._backToYear = Number(this.element[0].value) || this._getPickerYear();
                    this._setPickerYear(this._backToYear);
                    this._showYears();
                    var _label = this._getPickerYear();
        ...
        _showYearsClickHandler: function() {
        	if(this.options.isYearPicker) return;
            this._buttons.removeClass(_todayClass);
            var _elem = this.element;
            if (!this._backToYear) {
        ...
        _onYearClick: function(event) {
			if(this.options.isYearPicker) {
				var _year = event.data.year;
				var date = new Date(_year, 1);
				this.element.val(this._formatMonth(date)).blur();
				this.Close(event);
				return;
			}
